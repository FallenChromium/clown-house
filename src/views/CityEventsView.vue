<template>
  <n-space>
    ...
    <n-date-picker
      v-model:formatted-value="events_since"
      ...
      size="medium"
      type="date"
      :placeholder="'Select date since'"
    />
    <n-date-picker
      v-model:formatted-value="events_till"
      value-format="yyyy-MM-dd"
      :actions="[]"
      :clearable="true"
      format=""
        ...
        const actions = h(NSpace, {}, () => [
          h(
            NButton,
            {
              ...
              size: "small",
              onClick: () =>
                router.push({ name: "event", params: { id: row.id } }),
            },
            { default: () => "Edit" }
          ),
          h(
            NButton,
            {
              ...
              onClick: () => {
                if (row.id) {
                  DefaultService.deleteObjectEventsItemIdDelete(row.id).then(
                    async () => {
                      await fetch();
                    }
                  );
